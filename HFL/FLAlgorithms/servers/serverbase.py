...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        if not self.selected_users:
            return
        aggr_state = self.model.state_dict()
        for layer in aggr_state:
            aggr_state[layer] -= aggr_state[layer]
        total_samples = 0
        for user in self.selected_users:
            total_samples += user.train_samples
            state = user.model.state_dict()
            for layer in state:
                aggr_state[layer] += state[layer] * user.train_samples
        # aggr_state = aggr_state / total_samples
        for layer in aggr_state:
            aggr_state[layer] = aggr_state[layer].float() / float(total_samples)
        self.model.load_state_dict(aggr_state)
    ...
    def select_users(self, roun, num_users):
        ## TODO
        '''
        ...
        sub_idx = np.arange(numUsers, dtype=int)
        ...
        if numUsers <= 0:
            ...
            base = num_users
            picked_idx = sub_idx[base: min(base + num_users, numUsers)]
            return (np.array(self.users)[picked_idx]).tolist()
